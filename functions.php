...
function sb_login($email = '', $password = '', $user_id = '', $user_token = '') {
    ...
    if ($email && $password) {
        ...
        if (!$valid_login) {
            ...
            if ($verification_url) {
                $response = sb_curl($verification_url . (strpos($verification_url, '?') ? '&' : '?') . 'email=' . $email . '&password=' . $password);
...
function sb_js_admin() {
    ...
    $settings = [
        ...
        'notify-email-cron' => sb_get_setting('notify-email-cron'),
        'order-by-date' => sb_get_setting('order-by-date'),
        'max_file_size' => sb_get_server_max_file_size()
    ];
    $code = '<script>';
    if (defined('SB_DIALOGFLOW')) {
        $settings['open-ai-user-expressions'] = sb_get_multi_setting('open-ai', 'open-ai-user-expressions');
        $settings['smart-reply'] = sb_get_multi_setting('dialogflow-smart-reply', 'dialogflow-smart-reply-active') || sb_get_setting('ai-smart-reply'); // Deprecated: sb_get_multi_setting('dialogflow-smart-reply', 'dialogflow-smart-reply-active')
        $settings['open-ai-model'] = sb_open_ai_get_gpt_model();
    ...
    if (defined('SB_AECOMMERCE')) {
        $settings['aecommerce-panel-title'] = sb_get_setting('aecommerce-panel-title', 'Active eCommerce');
    }
    if ($settings['pusher']) {
    ...
    if (empty($SB_TRANSLATIONS[$string])) {
        if (defined('SB_DIALOGFLOW') && (sb_get_setting('dialogflow-multilingual-translation') || sb_get_multi_setting('google', 'google-multilingual-translation'))) { // Deprecated: sb_get_setting('dialogflow-multilingual-translation')
            ...
            if (!empty($response[0])) {
                ...
                $SB_TRANSLATIONS[$string] = $translation;
                $translations_file = [];
                $translations_file[$language_code] = ['front' => $SB_TRANSLATIONS];
...
function sb_get_front_settings() {
    ...
    $return = [
        ...
        'dialogflow-active' => sb_chatbot_active(true, false),
        'open-ai-active' => sb_chatbot_active(false, true),
    ...
    if ($return['welcome']) {
        $return['welcome-trigger'] = sb_get_multi_setting('welcome-message', 'welcome-trigger', 'load');
        $return['welcome-delay'] = sb_get_multi_setting('welcome-message', 'welcome-delay', 2000);
        $return['welcome-disable-office-hours'] = sb_get_multi_setting('welcome-message', 'welcome-disable-office-hours');
    }
    if ($return['queue']) {
        $return['queue-message'] = sb_get_multi_setting('queue', 'queue-message', '');
        $return['queue-response-time'] = sb_get_multi_setting('queue', 'queue-response-time', 5);
        $return['queue-sound'] = sb_get_multi_setting('queue', 'queue-sound');
    }
    if ($return['timetable']) {
        $return['timetable-type'] = sb_get_multi_setting('chat-timetable', 'chat-timetable-type');
        $return['timetable-hide'] = sb_get_multi_setting('chat-timetable', 'chat-timetable-hide');
        $return['timetable-disable-agents'] = sb_get_multi_setting('chat-timetable', 'chat-timetable-agents');
    }
    if ($return['wp']) {
    ...
    if (!empty($return['timetable-hide']) || !empty($return['timetable-type'])) {
        $return['timetable-message'] = [sb_t(sb_get_multi_setting('chat-timetable', 'chat-timetable-title')), sb_t(sb_get_multi_setting('chat-timetable', 'chat-timetable-msg'))];
    }
    if ($return['tickets']) {
        ...
        $return['tickets-welcome-message'] = sb_merge_fields(sb_t(sb_get_multi_setting('tickets-welcome-message', 'tickets-welcome-message-msg')));
        ...
        $return['tickets-hide'] = sb_get_setting('tickets-hide');
    ...
    if ($return['dialogflow-active'] || $return['open-ai-active']) {
        $return['dialogflow-human-takeover'] = sb_get_multi_setting('dialogflow-human-takeover', 'dialogflow-human-takeover-active');
        $return['dialogflow-welcome'] = sb_get_setting('dialogflow-welcome') || sb_get_multi_setting('google', 'google-welcome'); // Deprecated: sb_get_setting('dialogflow-welcome')
        $return['dialogflow-send-user-details'] = sb_get_setting('dialogflow-send-user-details') || sb_get_multi_setting('google', 'dialogflow-send-user-details'); // Deprecated: sb_get_setting('dialogflow-send-user-details')
        $return['dialogflow-departments'] = sb_get_setting('dialogflow-departments');
        $return['dialogflow-disable-tickets'] = sb_get_setting('dialogflow-disable-tickets');
        $return['dialogflow-office-hours'] = sb_get_setting('dialogflow-timetable');
...
function sb_csv($items, $header, $filename) {
    ...
    fclose($file);
    return sb_upload_path(true) . '/' . $filename;
}
...
function sb_execute_bot_message($name, $conversation_id, $last_user_message = false) {
    ...
    if ($valid && (($conversation_id && $name == 'offline') || sb_db_get('SELECT COUNT(*) AS `count` FROM sb_messages WHERE payload LIKE "{\"' . $name . '_message%" AND creation_time > "' . gmdate('Y-m-d H:i:s', time() - 864000) . '" AND conversation_id = ' . sb_db_escape($conversation_id, true))['count'] == 0)) {
        switch ($name) {
            case 'offline':
